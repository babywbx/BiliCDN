...
以下常用数据文件当前可通过 Cloudflare CDN 加速访问，适合程序直接引用：
...
其中 `nodes.meta.json`、`domains.live.txt`、`domains.video.txt` 是用途标签数据，适合脚本按直播/视频场景快速筛选。
...
| `nodes.meta.json` | 元数据 JSON 数组，包含域名、区域、域名族和用途标签 |
| `domains.live.txt` | 适合脚本使用的直播相关域名列表 |
| `domains.video.txt` | 适合脚本使用的视频相关域名列表 |
...
# 升级到包含 ec 标准节点的版本后，建议重新全量扫描而不是沿用旧 checkpoint
...
| `-o` | `data/nodes.json` | 输出文件（.json/.yml/.txt/.md 或元数据/列表文件名） |
| `-f` | （自动检测） | 强制格式（json/yaml/txt/md/meta/live/video） |
